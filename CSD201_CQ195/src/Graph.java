...
public class Graph {
    ...
    HashSet<Integer> lastVertex = new HashSet<>();

    void fvisit3(int vertex, RandomAccessFile f) throws Exception {
        if (!lastVertex.contains(vertex)) {
            f.writeBytes("  " + v[vertex]);
            lastVertex.add(vertex);
        }
    }
    ...
    void bfsTraversal(RandomAccessFile f) throws Exception {
    ...
    void bfs(int startVertex, RandomAccessFile f, boolean[] visited) throws Exception {
        Queue q = new Queue();
        int h;
    ...
    void findEulerCycle(int k, RandomAccessFile f) throws Exception {
        ...
        while (!stack.isEmpty()) {
            ...
            // Find a neighbor of u that has an edge in the graph
            int v;
            for (v = 0; v < n; v++) {
                if (tempGraph[u][v] > 0) {
                    break;
            ...
            if (v == n) {
                // If no neighbor with an edge is found, add u to the circuit
                circuit.add(u);
                stack.pop();
            } else {
                // Otherwise, remove the edge (u, v) from the graph and explore v next
                tempGraph[u][v]--;
                tempGraph[v][u]--;
                stack.push(v);
            }
        ...
        // Write the Euler cycle to the file
        for (int i = 0; i <= circuit.size() - 1; i++) {
            int vertex = circuit.get(i);
            fvisit(vertex, f);
        }
    }
    ...
    void f1() throws Exception {
         ...
         * disconnected graph. You are required to modify the bsfTraversal(...)
         * method provided in this class (to traverse all the vertices of each
         * connected component in the graph), and then call bfsTraversal(...) in
         * f1(). Notes: The number of connected components of a graph is also
         * the number of executions of the BFS (breadth-first search) algorithm
    ...
    void f2() throws Exception {
         ...
         * Starting from the fourth vertex (the D vertex has an index of 3),
         * ONLY display the D vertex and the next 3 vertices. Hint: use and
         * modify depth2(...), depth_rec2(...) (provided in this class)
         * appropriately, then call depth2(...) in f2(). The output f2() will be
         * written into the file 'f2.txt'. Therefore you should open this file
         * to see/test your code output. Example: With the graph provided in
         * 'data2.txt', the content of 'f2.txt' after running this method is: D
         * C B A H G F E I D C B A
         */
        //-------------------------------------------------------------------
        //------ Start your code here----------------------------------------
        depth2(3, f);
    ...
    void f3() throws Exception {
         ...
         * undirected multigraph with all vertices of even degree (G is the
         * Euler Graph). You are required to implement the findEulerCycle(...)
         * method (provided in this class), and then call findEulerCycle(...) in
         * f3() to write a line containing an Euler cycle of the graph G (starts
         * from A, index is 0) to the file f1.txt. Hint: use the algorithm for
         * finding an Euler cycle from the vertex X using stack to perform and
         * fvisit(...) to save the result. The output f3() will be written into
         * the file 'f3.txt'. Therefore you should open this file to see/test
         * your code output. Example: With the graph provided in 'data3.txt',
         * the content of 'f3.txt' after running this method is: A B D C B A
         */
        //-------------------------------------------------------------------
        //------ Start your code here----------------------------------------
    ...
    void f4() throws Exception {
         ...
         * vertex A (the vertex A has an index of 0) to vertex D (the index of
         * the vertex D is 3), then write 2 lines to the file f4.txt: (1) Line
         * 1: contains vertices selected into the set S until the vertex D is
         * selected, and (2) Line 2: contains vertices in shortest path from A
         * to D. Hint: use and modify dijkstra(...), dijkstra2(...),
         * pathDijkstra(...), fvisit(...), fvisit2(...), fvisit3(...) (provided
         * in this class) appropriately, then call dijkstra(...) in f4().
         ...
         * 'f4.txt' after running these methods is: A B H G F C I D A B C D
         */
        //-------------------------------------------------------------------
        //------ Start your code here----------------------------------------
